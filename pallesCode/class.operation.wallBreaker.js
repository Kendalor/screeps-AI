...
module.exports = class{
        ...
        static run(id){
			//Memory.operations[id].nearest_spawnId = Game.spawns['Spawn1'].id;
            ...
            // BUILD CREEPS UNTIL SQUAD SIZE REACHED
				...
				if(Memory.operations[id].roomName != Game.flags[Memory.operations[id].flagName].pos.roomName)
					Memory.operations[id].roomName = Game.flags[Memory.operations[id].flagName].pos.roomName;
            }
        }
                ...
                for(var sLeader in Memory.operations[id].squads){
                    ...
                    if(Game.creeps[sLeader].pos.roomName == Memory.operations[id].roomName){
                        ...
                        if(enemies == undefined){
							...
							enemies=Game.creeps[sLeader].room.find(FIND_STRUCTURES,{filter: (structure) => structure.structureType == STRUCTURE_WALL || structure.structureType == STRUCTURE_RAMPAGE}); // ALTERNATIVE FOR DEMOLISHING WALLS - NOT WORKING PROPERLY ATM
							//Game.creeps[sLeader].say(enemies);
                        }
						if (enemies == undefined){
							enemies=Game.creeps[sLeader].room.find(FIND_STRUCTURES,{filter: (structure) => structure.structureType == STRUCTURE_TOWER || structure.structureType == STRUCTURE_SPAWN ||structure.structureType == STRUCTURE_EXTENSION});
						}
                        if(enemies !=undefined){
                            target=Game.creeps[sLeader].pos.findClosestByPath(enemies);
							if (target != null)
								Game.creeps[sLeader].say(target.structureType);
							
							err=Game.creeps[sLeader].dismantle(target);
							//err=Game.creeps[sLeader].attack(target);
							if (Game.creeps[sLeader].hits < Game.creeps[sLeader].hitsMax/2)
								Game.creeps[sLeader].heal(Game.creeps[sLeader]);
							/*
                            if(err == ERR_NOT_IN_RANGE){
                                err == Game.creeps[sLeader].moveTo(target);
                            }
							*/
        ...
        */
        ...
        static init(roomName,flag){
            if(!Game.flags[flag].memory.operation_id){
				...
				//COST 11xMOVE = 550 + 10xWORK= 1000 + 1xHEAL=250 =1800
				Memory.operations[this.id].default_Abody=[MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,HEAL];
