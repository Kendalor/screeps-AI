        ...
        static buildAndRunCreeps(id){
            ...
            for(var i in Memory.operations[id].sources){
                ...
                if(!Memory.operations[id].sources[i].ticksToSource){
                    ...
                    Memory.operations[id].sources[i].carry_parts=Math.round(Memory.operations[id].sources[i].ticksToSource*2/5);
                    Memory.operations[id].sources[i].min_haulers=Math.round(Memory.operations[id].sources[i].ticksToSource*2/5/18);
        ...
        static buildRoadAndContainer(id){
            if(!Memory.operations[id].sources){//If this room has no sources memory yet
                ...
                for(var s_id in sources){
                 ...
                 var path=PathFinder.search(source.pos,{pos: storage.pos, range: 1},{plainCost: 2, swampCost: 2,
