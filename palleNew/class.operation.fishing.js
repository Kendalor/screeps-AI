...
module.exports = class{
        ...
        static run(id){
			...
			// TEST BODY :
			var creep_body = [TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,HEAL];
            if(!this.checkForDelete(id)){ // RUN ONLY IF APPLICABLE
				...
				if(Object.keys(Memory.operations[id].members).length < Memory.operations[id].size && !Memory.operations[id].members.assembled){
					...
					if(Game.getObjectById(Memory.operations[id].nearest_spawnId).canCreateCreep(creep_body, undefined, {role: 'fisher', operation: id, target: Memory.operations[id].flagName}) == OK){
						var name=Game.getObjectById(Memory.operations[id].nearest_spawnId).createCreep(creep_body,undefined,{role: 'fisher', operation_id: id, target: Memory.operations[id].flagName});
						Memory.operations[id].members[name]= 'fisher';
				...
				for(var cr in Memory.operations[id].members){
					if(!Game.creeps[cr].spawning && Game.creeps[cr]){
						if(Memory.operations[id].assembled==false){
							...
							console.log('Running Fishing for '+cr);
        ...
        static init(roomName,flag){
            if(!Game.flags[flag].memory.operation_id){
                ...
                Memory.operations[this.id].type='fishing';
