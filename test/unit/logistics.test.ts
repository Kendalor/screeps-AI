...
import { colony, containerAt, sourceAt } from "../fixtures";

describe("desiredMinerCount", () => {
  it("wants exactly one miner for a single source when no haulers are alive yet", () => {
    expect(desiredMinerCount(colony({ sources: [sourceAt(20, 10)], census: {} }))).toBe(1);
  });
  ...
  it("caps headcount by live hauler count once a hauler exists", () => {
    ...
    expect(desiredMinerCount(colony({ sources: [sourceAt(20, 10)], census: { hauler: 1 } }))).toBe(1);
  });
  ...
  it("stops applying the one-miner floor once a hauler is alive", () => {
    ...
    expect(desiredMinerCount(colony({ sources: [sourceAt(20, 10)], census: { hauler: 3 } }))).toBe(3);
  });

  it("reapplies the one-miner floor after all haulers die", () => {
    expect(desiredMinerCount(colony({ sources: [sourceAt(20, 10)], census: { hauler: 0 } }))).toBe(1);
  });
  ...
  it("caps headcount by the source's open adjacent tiles, even with haulers to spare", () => {
    ...
    expect(desiredMinerCount(colony({ sources: [enclosed], census: { hauler: 5 } }))).toBe(1);
  });

  it("wants fewer, bigger miners as energy capacity grows", () => {
    const small = colony({ sources: [sourceAt(20, 10)], census: { hauler: 5 }, energyCapacity: 300 });
    const large = colony({ sources: [sourceAt(20, 10)], census: { hauler: 5 }, energyCapacity: 1000 });
