...
// Settler/self-sufficiency demand for a claimed-but-spawnless target now lives entirely in Bootstrap's
// noSpawnRequests (operations/bootstrap.ts, test/unit/operations/bootstrap.test.ts) — the target shows up
// as its own real Colony once claimed, and that Colony's own Bootstrap operation handles it directly, no
// threading through the sponsor needed. Colony's constructor still threads targetEnergyCapacity through
// to Colonize itself, but only to stop requesting the colonizer once the claim has landed.
describe("Colony: colonize target energy capacity threading", () => {
  it("keeps requesting a colonizer when the target isn't yet a real colony (claim hasn't landed)", () => {
    ...
    const colonize = c.operations.find(op => op.kind === "colonize");
    const colonizerRequests = colonize!.desiredCreeps(sponsor).filter(r => r.memory.role === "colonizer");
    expect(colonizerRequests).toHaveLength(1);
  });

  it("stops requesting a colonizer once the target appears in allSnapshots as a real colony", () => {
    const sponsor = colonySnap({ name: "W1N1", energyCapacity: 650, colonizing: ["W5N5"] });
    const target = colonySnap({ name: "W5N5", energyCapacity: 300 });
    const c = colony(sponsor, [sponsor, target]);
    const colonize = c.operations.find(op => op.kind === "colonize");
    const colonizerRequests = colonize!.desiredCreeps(sponsor).filter(r => r.memory.role === "colonizer");
    expect(colonizerRequests).toEqual([]);
  });

  it("does not confuse a different colony's presence in allSnapshots for the actual target's", () => {
    const sponsor = colonySnap({ name: "W1N1", energyCapacity: 650, colonizing: ["W5N5"] });
    // A third, unrelated colony sits in allSnapshots — must not read as W5N5 having been claimed.
