// A colony that has lost every creep spawns its way back via the RECOVERY branch.
//
// The room is given storage full of energy specifically so recovery chooses `supply` — a cold
// room with sources would spawn a bootstrap via the normal quota path regardless, proving nothing.
// Asserting on the role (not merely "a creep exists") ties the test to the branch under test.
...
beforeAll(async () => {
  ...
  // Recovery is about losing what you had, so the room starts established rather than a fresh spawn.
