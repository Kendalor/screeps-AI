...
export class Upgrader extends CreepRole {
	...
	public static getBody(spawn: StructureSpawn): BodyPartConstant[] {
		const energyCap = spawn.room.energyCapacityAvailable - 200;
		let partArray: BodyPartConstant[] = [CARRY,CARRY,MOVE,MOVE];
		const fullSets = Math.max(1, Math.floor(energyCap/250));
