  ...
  public runLegacy(): void {
    for(const i in this.towerList){
      if(this.towerList[i] !== null) {
        ...
        let minHits: number = this.towerList[i].room.memory.structureHitsMin;
        const closestHostile = this.towerList[i].pos.findClosestByRange(FIND_HOSTILE_CREEPS,{filter: (hostile) =>
        ...
        if(closestHostile) {
          ...
          let closestDamagedStructure = this.towerList[i].pos.findClosestByRange(FIND_STRUCTURES, {
