...
// miner container), it sweeps small dropped/tombstone/ruin piles it passes near, so loose energy doesn't
...
// A dropped Resource exposes .amount; a tombstone/ruin exposes .store. Read whichever the pile carries.
function energyOf(pile: Resource | Tombstone | Ruin): number {
  const asDrop = pile as { amount?: number };
  if (asDrop.amount !== undefined) return asDrop.amount;
  return (pile as Tombstone | Ruin).store.getUsedCapacity(RESOURCE_ENERGY) ?? 0;
}
...
export function pickSweepPile(creep: Creep): Resource | Tombstone | Ruin | undefined {
  ...
  const ruins = creep.pos
    .findInRange(FIND_RUINS, SWEEP_RADIUS)
    .filter(r => r.store.getUsedCapacity(RESOURCE_ENERGY) >= SWEEP_FLOOR);
...
export function sweepEnRoute(creep: Creep): boolean {
  ...
  if (creep.pos.isNearTo(pile.pos)) {
    ...
    else creep.withdraw(pile as Tombstone | Ruin, RESOURCE_ENERGY);
    return true;
  }
