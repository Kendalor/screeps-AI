...
// mining and expansion have data to decide on. Ported from legacy's OperationScoutingManager, reduced
// to the rewrite's shape — no persisted todo ledger, no pause/wakeup scheduler. The "what rooms exist
// near me" graph is walked once at the snapshot boundary (colony.scoutTargets); this operation reads
// it as plain data, decides which are stale, and sizes the scout fleet.
//
// Pure — reads the snapshot, returns plain data, never touches Game.*/Memory. It drives scouts
// entirely through intents (recordScout, setScoutTarget): execute.ts does the live-room read and
...
export class Scouting extends Operation {
   ...
   * MAX_SCOUTS. Zero when every reachable room is fresh — the fleet is allowed to die off by
   * attrition until an observation goes stale and re-opens demand.
   *
   * A headcount, not a per-target deficit: which room each scout walks to is the scout's own
   * decision (nearest unscouted), so the operation only sizes the fleet.
   */
