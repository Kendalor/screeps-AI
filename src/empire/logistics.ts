...
export function runEmpireLogisticsPass(colonyNames: readonly string[]): Intent[] {
  ...
  // Built from baseTargetFor rather than reading BOOST_TARGETS directly, so LIQUIDATION_MODE (now owned by
  // boostTargets.ts, see its own doc) is honored automatically: with it on, every target collapses to 0, so
  // EVERY colony's full current stock reads as pure surplus and none reads as a deficit — matchEmpireRequests
  // then has no opposite-signed side to pair against for these resources at all, so colony-to-colony
  // redistribution becomes a structural no-op on its own (nothing special-cased here to "skip" it) and the
  // entire stockpile flows straight through to `leftover`, ready for marketFallback's now-looser sell floor
  // to actually clear it. Recomputed fresh every pass rather than cached, same "no persisted intermediate
  // state" rule decision 11 already applies to `requests`/`leftover` themselves.
