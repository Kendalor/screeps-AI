...
// convention, applying to src/logistics/ only (see the ADR's own scope note). One registration source for
// now: a miner's source container, matching the PRD's scoped acceptance criteria. Mineral registration
// (unconditional, no threshold) is gh #48 — not built here.
...
// ADR 0008's registration-asymmetry section). Energy only; a mineral container has no such gate (gh #48).
